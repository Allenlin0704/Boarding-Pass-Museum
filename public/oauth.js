(()=>{
  ...
  if(!host)return;
  const labels={apple:'Apple',microsoft:'Microsoft',github:'GitHub'};
  ...
  fetch(`${API}/api/oauth/providers`,{credentials:'include'}).then(response=>response.json()).then(status=>{
    ...
    for(const provider of ['github','apple','microsoft']){
    ...
    host.hidden=!status.github&&!status.apple&&!status.microsoft;
